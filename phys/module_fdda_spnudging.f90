 ...
 DO ij = 1 , num_tiles
   ...
   ENDIF
     ...
     IF ( ktrop > 0 ) THEN
       ...
       DO j=jts,jtf
       DO i=its,itf
           wpbl(i, ktrop, j, 3) = wpbl(i, ktrop, j, 3) * 0.5
       ENDDO
       ENDDO
   ...
   IF( if_no_pbl_nudging_q == 1 ) THEN
     ...
     DO i=its,itf
       ...
       DO k=kts,ktf
          wpbl(i, k, j, 5) = max ( min ( float(k-kpbl) / float(dk_zfac_q) , 1. ) , 0.)
       ENDDO

     ENDDO
     ENDDO

   ELSEIF( if_zfac_q == 1 ) THEN

     DO j=jts,jtf
     DO k=kts,ktf
     DO i=its,itf
          wpbl(i, k, j, 5) = max ( min ( float(k-k_zfac_q) / float(dk_zfac_q) , 1. ) , 0.)
     ENDDO
     ENDDO
     ENDDO
   ...
   ENDIF
     ...
     IF ( ktrop > 0 ) THEN
       ...
       DO j=jts,jtf
       DO i=its,itf
           wpbl(i, ktrop, j, 5) = wpbl(i, ktrop, j, 5) * 0.5
       ENDDO
       ENDDO
...
IF(gq > 0. ) then
 ...
 DO ij = 1 , num_tiles
   ...
   DO i=its,ite
     ...
     grid%dif_analysis(i,k,j) = val_analysis - qv3d(i,k,j)
 ...
 DO ij = 1 , num_tiles
   ...
   DO k=kts,kte
   DO i=its,ite
     RQVNDGDTEN(i,k,j) = gq * wpbl(i,k,j,5) * wzfac(k,5) * tfac * &
                        grid%dif_analysis(i,k,j)
   ENDDO
   ENDDO
   ENDDO
...
'In grid FDDA, Gq must be positive.')
     ...
     IF( guv > 0.0 .AND. if_no_pbl_nudging_uv == 1 ) THEN
        ...
        CALL wrf_message(TRIM(wrf_err_message))
             IF( dk_zfac_uv < 1 ) CALL wrf_error_fatal3("<stdin>",1415,&
'In spectral nudging, dk_zfac_uv must be greater or equal than 1.')
     ELSEIF( guv > 0.0 .AND. if_zfac_uv == 1 ) THEN
        ...
        CALL wrf_message(TRIM(wrf_err_message))
             IF( dk_zfac_uv < 1 ) CALL wrf_error_fatal3("<stdin>",1421,&
'In spectral nudging, dk_zfac_uv must  be greater or equal than 1.')       
     ENDIF
     ...
     IF( gt > 0.0 .AND. if_no_pbl_nudging_t == 1 ) THEN
        ...
        CALL wrf_message(TRIM(wrf_err_message))
             IF( dk_zfac_t < 1 ) CALL wrf_error_fatal3("<stdin>",1430,&
'In spectral nudging, dk_zfac_t must be greater or equal than 1.')
     ELSEIF( gt > 0.0 .AND. if_zfac_t == 1 ) THEN
        ...
        CALL wrf_message(TRIM(wrf_err_message))
            IF( dk_zfac_t < 1 ) CALL wrf_error_fatal3("<stdin>",1436,&
'In spectral nudging, dk_zfac_t must be greater or equal than 1.')
     ENDIF
     ...
     IF( gph > 0.0 .AND. if_no_pbl_nudging_ph == 1 ) THEN
        ...
        CALL wrf_message(TRIM(wrf_err_message))
            IF( dk_zfac_ph < 1 ) CALL wrf_error_fatal3("<stdin>",1445,&
     ...
     ELSEIF( gph > 0.0 .AND. if_zfac_ph == 1 ) THEN
        WRITE(wrf_err_message,'(a,i1,a,i3)') &
          'D0',id,' Spectral nudging for geopotential is turned off below layer', &
           k_zfac_ph
        CALL wrf_message(TRIM(wrf_err_message))
            IF( dk_zfac_ph < 1 ) CALL wrf_error_fatal3("<stdin>",1452,&
'In spectral nudging, dk_zfac_ph must be greater or equal than 1.')
     ENDIF
