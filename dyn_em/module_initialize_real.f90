...
grid%tracer_btys,grid%tracer_btye,grid%nba_mij,grid%nba_rij,grid%chem &
...
"--- ERROR: specified eta_levels are not uniformly decreasing from 1.0 to 0.0")
         ...
         DO k=kts, kte
            ...
            ELSE
               ...
               CALL wrf_error_fatal3("<stdin>",1609,&
...
oops2=oops2+1
               ...
               ELSE
                  ...
                  CALL wrf_error_fatal3("<stdin>",3418,&
...
'initialize_real: hypsometric_opt should be 1 or 2' )
      ...
      IF ( config_flags%use_aero_icbc .AND. ( flag_qnwfa .EQ. 1 ) .AND. &
         ...
         do i = its, min(ide-1,ite)
            ...
            airmass = 1./grid%alt(i,1,j) * z1 * config_flags%dx*config_flags%dy          
