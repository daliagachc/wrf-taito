  ...
  ips, ipe, jps, jpe, kps, kpe )
   ...
   IF( couple ) THEN
     ...
     DO i = max(ids,ips),min(ide-1,ipe)
       ...
       muwt_2(i,k,j) = ((grid%c1f(k)*grid%Mub(i,j)+grid%c2f(k)) + (grid%c1f(k)*grid%Mu_2(i,j)))/grid%msfty(i,j) 
     ENDDO
     ENDDO
     ENDDO
     ...
     DO k = kps,kpe-1
     DO i = max(ids,ips),min(ide-1,ipe)
       muut_2(i,k,j) = 0.5*((grid%c1h(k)*grid%mub(i,j)+grid%c2h(k))+(grid%c1h(k)*grid%mub(i-1,j)+grid%c2h(k)) + (grid%c1h(k)*grid%mu_2(i,j)) + (grid%c1h(k)*grid%mu_2(i-1,j)))/grid%msfuy(i,j) 
       muvt_2(i,k,j) = 0.5*((grid%c1h(k)*grid%mub(i,j)+grid%c2h(k))+(grid%c1h(k)*grid%mub(i,j-1)+grid%c2h(k)) + (grid%c1h(k)*grid%mu_2(i,j)) + (grid%c1h(k)*grid%mu_2(i,j-1)))/grid%msfvx(i,j) 
     ENDDO
     ENDDO
     ENDDO
     ...
     IF ( config_flags%nested .or. config_flags%specified .or. config_flags%polar ) THEN
       ...
       IF ( jpe .eq. jde ) THEN
         ...
         DO k = kps,kpe-1
         DO i = max(ids,ips),min(ide-1,ipe)
           muvt_2(i,k,j) = ((grid%c1h(k)*grid%mub(i,j-1)+grid%c2h(k)) + (grid%c1h(k)*grid%mu_2(i,j-1)))/grid%msfvx(i,j) 
         ENDDO
         ENDDO
       ENDIF
       IF ( ipe .eq. ide .AND. .NOT. config_flags%periodic_x ) THEN
         ...
         DO j = max(jds,jps),min(jde-1,jpe)
         DO k = kps,kpe-1
           muut_2(i,k,j) = ((grid%c1h(k)*grid%mub(i-1,j)+grid%c2h(k)) + (grid%c1h(k)*grid%mu_2(i-1,j)))/grid%msfuy(i,j) 
         ENDDO
         ENDDO
       ENDIF
   ...
   ELSE
     ...
     DO i = max(ids,ips),min(ide-1,ipe)
       ...
       muwt_2(i,k,j) = grid%msfty(i,j)/((grid%c1f(k)*grid%Mub(i,j)+grid%c2f(k)) + (grid%c1f(k)*grid%Mu_2(i,j))) 
     ENDDO
     ENDDO
     ENDDO
     ...
     DO k = kps,kpe-1
     DO i = max(ids,ips),min(ide-1,ipe)
       muut_2(i,k,j) = 2.*grid%msfuy(i,j)/((grid%c1h(k)*grid%mub(i,j)+grid%c2h(k))+(grid%c1h(k)*grid%mub(i-1,j)+grid%c2h(k)) + (grid%c1h(k)*grid%mu_2(i,j)) + (grid%c1h(k)*grid%mu_2(i-1,j))) 
     ENDDO
     ENDDO
     ENDDO
     ...
     DO k = kps,kpe-1
     DO i = max(ids,ips),min(ide-1,ipe)
       muvt_2(i,k,j) = 2.*grid%msfvx(i,j)/((grid%c1h(k)*grid%mub(i,j)+grid%c2h(k))+(grid%c1h(k)*grid%mub(i,j-1)+grid%c2h(k)) + (grid%c1h(k)*grid%mu_2(i,j)) + (grid%c1h(k)*grid%mu_2(i,j-1))) 
     ENDDO
     ENDDO
     ENDDO
     ...
     IF ( config_flags%nested .or. config_flags%specified .or. config_flags%polar ) THEN
       ...
       IF ( jpe .eq. jde ) THEN
         ...
         DO k = kps,kpe-1
         DO i = max(ids,ips),min(ide-1,ipe)
           muvt_2(i,k,j) = grid%msfvx(i,j)/((grid%c1h(k)*grid%mub(i,j-1)+grid%c2h(k)) + (grid%c1h(k)*grid%mu_2(i,j-1))) 
         ENDDO
         ENDDO
       ENDIF
       IF ( ipe .eq. ide .AND. .NOT. config_flags%periodic_x ) THEN
         ...
         DO j = max(jds,jps),min(jde-1,jpe)
         DO k = kps,kpe-1
           muut_2(i,k,j) = grid%msfuy(i,j)/((grid%c1h(k)*grid%mub(i-1,j)+grid%c2h(k)) + (grid%c1h(k)*grid%mu_2(i-1,j))) 
         ENDDO
         ENDDO
       ENDIF
